<script>
  ...
  export default {
    ...
    computed: {
      ...mapGetters('grouper', ['selected', 'grouperProjects'])
    },
    ...
    methods: {
      sendReportByEmail () {
        ...
        this.email.url = paths.app + '/grouperConsult/reportById/' + this.selected.id

        if (this.emailType === 'Imagem') {
          // this.email.url = paths.app + '/grouperConsult/report/' + this.selected.id
      ...
      closeModalShowReport () {
        this.$router.push({ name: 'showByIdGrouperConsult', params: { grouper: this.selected.id } })
      }
    },

    mounted () {
      this.email.subject = '[Status Agrupador] ' + this.selected.name
    }
        ...
        <div class="col-xs-12 well well-sm oi-well">
          <div class="col-sm-1" style="margin:0; border:0; padding:5px" v-show="selected.type === 'STATUS'">
              ...
              <div class="text-left" style="padding-bottom:5px">
                ...
                <img alt="Farol Amarelo" src="../../../asset/image/amarelo-sm.png" v-show="selected.trafficLight === 'AMARELO'">
                <img alt="Farol Vermelho" src="../../../asset/image/vermelho-sm.png" v-show="selected.trafficLight === 'VERMELHO'">
              </div>
          </div>
          ...
          <div class="col-xs-12" style="margin:0; border:0; padding:5px" v-show="selected.type === 'STATUS'">
            ...
            <div>
              <label 
                class="fd-content" 
                v-html="selected.executiveSummary ? selected.executiveSummary : 'Sem dados!'"
              />
            </div>
          </div>
