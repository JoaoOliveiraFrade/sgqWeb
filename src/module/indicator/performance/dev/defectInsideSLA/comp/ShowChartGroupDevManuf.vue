<script>
  ...
  export default {
    ...
    computed: {
      ...mapGetters('indPerfDevDefectInsideSLA', ['groupDevManuf', 'byDevManufGroupSystem', 'chartTitle'])
    },

    methods: {
      ...mapActions('indPerfDevDefectInsideSLA', ['setChartFilter']),
      ...
      //     <b>{point.name}</b><br>
      //     Dentro SLA: {point.qtyInsideSLA:.0f} ({point.percWithinSLA:.2f}%{point.percTotalDefectsWithinSLA})<br>
      ...
      setChartParam () {
        ...
        this.chartParam.tooltip.pointFormat = `
          <b>{point.name}</b><br>
          Dentro SLA: {point.percWithinSLA:.2f}% ({point.qtyInsideSLA:.0f})<br>
