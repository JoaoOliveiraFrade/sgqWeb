...
export const load = ({ commit, state, rootState }) => {
  ...
  return new Promise((resolve, reject) => {
    services.load({
      selectedDevManufs: rootState.indPerfDevDefectInsideSLA.selectedDevManufs.map(i => i.id),
      selectedSystems: rootState.indPerfDevDefectInsideSLA.selectedSystems.map(i => i.id),
      selectedProjects: rootState.indPerfDevDefectInsideSLA.selectedProjects.map(i => i.subproject + i.delivery)
    })
    .then(
      r => {
