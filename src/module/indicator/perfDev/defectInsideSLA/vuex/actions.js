...
export const load = ({ commit, state, rootState }) => {
  ...
  return new Promise((resolve, reject) => {
    services.load({
      selectedDevManufs: rootState.indicatorPerfDevDefectInsideSLA.selectedDevManufs.map(i => i.id),
      selectedSystems: rootState.indicatorPerfDevDefectInsideSLA.selectedSystems.map(i => i.id),
      selectedProjects: rootState.indicatorPerfDevDefectInsideSLA.selectedProjects.map(i => i.subproject + i.delivery)
    })
    .then(
      r => {
