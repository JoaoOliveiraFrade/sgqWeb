  ...
  export default {
    ...
    methods: {
      loadParameters () {
        ...
        this.parameters.title.text = 'Em Teste C/Risco Alto'
        this.parameters.yAxis.title.text = 'Qte<br>Entrega'

        this.parameters.plotOptions.gauge.dataLabels.useHTML = true
        this.parameters.plotOptions.gauge.dataLabels.format = '<center>{point.y:.0f}<br>' + averageHighRiskProgress + '%</center>'

        this.parameters.yAxis.max = this.value.candidate

        this.parameters.yAxis.plotBands = [
          {from: 0, to: this.value.highRiskProgress, color: '#00CC00'},
          {from: this.value.highRiskProgress, to: this.value.candidate, color: '#FF3300'}
        ]
        ...
        this.parameters.series = [ { name: 'testCompleted', colorByPoint: true, data: [ this.value.highRiskProgress ] } ]
      },
      drawChart () {
